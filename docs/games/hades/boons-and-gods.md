...
The structure is identical to Slay the Spire's `card-draft-with-skip` pattern transposed to real-time action combat. **The same draft mental-model that works for Spire's tactical combat works for Hades's frantic action combat, because the offer happens *between* combats, in a slow chamber, with full text and tooltip support.**
