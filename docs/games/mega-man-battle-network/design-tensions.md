...
This is the **design thesis as a sentence.** The system is built so that strategy-driven wins feel better than power-driven wins; the constraint structure (folder size, code rule, custom gauge, PA recipes) is the machinery that makes strategy load-bearing.
