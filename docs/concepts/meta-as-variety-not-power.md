...
| **[Cult of the Lamb](/games/cult-of-the-lamb/combat-and-loadout)** | Tarot codex (85+ cards in draw pool), weapon/curse pool unlocks, Fleeces (persistent run-shape modifiers), Relics, follower forms, Doctrines | Some power unlocks exist — Heart Containers via Sermon-tree ("Hearts of the Faithful"), Crown abilities from Bishop kills, max-HP via Ratoo / Hathor relic. A **mixed stance**: per-run loadout is pure variety (Tarots reset every Crusade), but the durable Crown / Sermon / Hearts ladder *is* power growth |
