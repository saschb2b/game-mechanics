...
| **[Cult of the Lamb](/games/cult-of-the-lamb/divine-inspiration)** | Divine Inspiration tree (~6 building branches gated by Cult Level rows) · Doctrine tree (4+ permanent policy branches) · Sermon tree (per-Crusade modifiers) · Tarot codex · Weapon/Curse/Fleece/Relic pools · Follower-form collection | Each axis spends a different currency: Divine Inspiration (Devotion → points) for buildings, Faith for Doctrines, Sermons for skill nodes, dungeon Talisman fragments for Crown perks. Adding *Sins of the Flesh* in 2024 layered a 7th axis (Sin / procreation) without invalidating the prior six | Two-axis gating in the tech tree (category × Cult-Level row) plus six other parallel meta-graphs: unusually nested for a 20-hour indie scope |
