...
| **[Balatro](/games/balatro/jokers)** | **5-slot Joker loadout from a pool of 150+; slot order matters (Jokers fire left-to-right)** | Jokers (passive triggers: flat adders, scalers, conditional triggers, chain meta-jokers like Blueprint) | 5 slots; expansion via Negative editions (rare) and Antimatter voucher; Spectral cards can destroy all Jokers as a "swap" cost | A Joker dropped is 5 turns of build identity gone. Slot *order* doubles the optimization space: Blueprint copies the Joker to its right, so the chain composition matters |
| **[Clair Obscur: Expedition 33](/games/clair-obscur-expedition-33/pictos-and-luminas)** | **Numeric, level-coupled: Lumina Points per character (≈ character level, permanently extendable via a consumable)** | Luminas: passives learned by winning 4 battles with the source Picto equipped, costing 1–40 points each | Total points only; **no slot cap**, so the budget is the entire constraint | A 40-point Energy Master eats what could be twenty 1–2 point utilities; the budget grows every level, so the trade is re-litigated all game |
...
| Hollow Knight | Nier: Automata | Pokémon RSE | Clair Obscur: Expedition 33 |
|---|---|---|---|
| ![Hollow Knight charm screen: 5 of 11 notches used, full grid of charms below](/images/hollow-knight/hk-charms-equip.jpg) | ![Nier Automata chip categories: Storage Used 68/72](/images/nier-automata/chip-categories.jpg) | ![Pokémon battle moves: Torchic with 4 typed moves and PP counters](/images/pokemon-rse/battle-moves.jpg) | ![Expedition 33 Pictos screen with Dodger tooltip and Lumina counter](/images/clair-obscur-expedition-33/pictos-equip-dodger.jpg) |
| 11 notches, ~45 charms with integer costs. Pure numeric. | 72 storage, chip costs 1–17. Same numeric form: but the HUD elements (HP bar, minimap, damage numbers) are *also* chips in the same budget. | 4 move slots, hundreds of typed moves. The smallest-grain budget: and the constraint that's anchored Pokémon battling for ~30 years. | Lumina Points (bottom right, 0/2): a budget that starts near zero and grows every level. Costs 1–40 per passive, no slot cap. The only entry here where budget *items* are learned from other equipment. |
