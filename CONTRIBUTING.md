...
How to add new entries and the conventions to follow.
...
| `patterns.md` | yes | Quick reference table of patterns this game exemplifies. Curated patterns link to `/concepts/<pattern>`; uncurated patterns are plain `` `pattern-name` `` (no link) |
...
**Curated concept pages do not auto-update.** You must edit them by hand.
...
Fix any dead-link errors before considering the entry done. Common cause: a prose link to `/concepts/<x>` where no concept page exists — either create the concept page or drop the link.
...
To find candidates, grep the repo for `patterns:` frontmatter entries that appear in two or more games' files but don't yet have a `docs/concepts/<slug>.md`.
...
Game pages (hand-written)
  ├─ frontmatter: patterns: [a, b, c]   (machine reference; not crawled to render anything)
  └─ prose: links to /concepts/<x> for curated patterns; uncurated patterns are
            written as plain `pattern-name` (no link)
...
When game X is added with pattern Y, and Y already has a curated concept page, *you* must add a row to that concept page's variants table. Concept-page rows are bespoke prose ("how does game X specifically instantiate this pattern?") — the artisanal layer.
